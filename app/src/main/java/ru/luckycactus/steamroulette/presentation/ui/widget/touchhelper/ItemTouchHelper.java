package ru.luckycactus.steamroulette.presentation.ui.widget.touchhelper;
 ...
 *
 * EXTRA:
 * Added <code>swipe</code> method and constructor parameter <code>translationFraction</code> to
...
public class ItemTouchHelper extends RecyclerView.ItemDecoration
    ...
    private final float translationFraction;
    ...
    public ItemTouchHelper(@NonNull Callback callback, float translationFraction) {
        mCallback = callback;
        this.translationFraction = translationFraction;
    }
        ...
        if (mSelected != null) {
            ...
            if (prevSelected.itemView.getParent() != null) {
                ...
                switch (swipeDir) {
                    ...
                    case END:
                        targetTranslateY = 0;
                        targetTranslateX = Math.signum(mDx) * mRecyclerView.getWidth() * translationFraction;
                        break;
                    case UP:
                    case DOWN:
                        targetTranslateX = 0;
                        targetTranslateY = Math.signum(mDy) * mRecyclerView.getHeight() * translationFraction;
                        break;
                    default:
                        targetTranslateX = 0;
                        targetTranslateY = 0;
    ...
    public void swipe(ViewHolder selected, int swipeDir) {
        final int movementFlags = mCallback.getAbsoluteMovementFlags(mRecyclerView, selected);

        final int swipeFlags = (movementFlags & ACTION_MODE_SWIPE_MASK)
                >> (DIRECTION_FLAG_COUNT * ACTION_STATE_SWIPE);

        if ((swipeFlags & swipeDir) == 0)
            return;
        ...
        switch (swipeDir) {
            case LEFT:
                targetTranslateY = 0;
                targetTranslateX = -mRecyclerView.getWidth() * translationFraction;
                break;
            case RIGHT:
                targetTranslateY = 0;
                targetTranslateX = mRecyclerView.getWidth() * translationFraction;
            ...
            case UP:
                targetTranslateX = 0;
                targetTranslateY = -mRecyclerView.getHeight() * translationFraction;
                break;
            case DOWN:
                targetTranslateX = 0;
                targetTranslateY = mRecyclerView.getHeight() * translationFraction;
        ...
        final float currentTranslateX = mTmpPosition[0];
        final float currentTranslateY = mTmpPosition[1];
        final RecoverAnimation rv = new RecoverAnimation(selected, ANIMATION_TYPE_SWIPE_SUCCESS,
                ACTION_STATE_SWIPE, currentTranslateX, currentTranslateY,
                targetTranslateX, targetTranslateY) {
            @Override
            public void onAnimationEnd(Animator animation) {
                super.onAnimationEnd(animation);
                ...
                if (swipeDir <= 0) {
                    // this is a drag or failed swipe. recover immediately
                    mCallback.clearView(mRecyclerView, selected);
                    // full cleanup will happen on onDrawOver
                } else {
                    // wait until remove animation is complete.
                    mPendingCleanup.add(selected.itemView);
                    mIsPendingCleanup = true;
                    if (swipeDir > 0) {
                        // Animation might be ended by other animators during a layout.
                        // We defer callback to avoid editing adapter during a layout.
                ...
                // removed from the list after it is drawn for the last time
                if (mOverdrawChild == selected.itemView) {
                    removeChildDrawingOrderCallbackIfNecessary(selected.itemView);
                }
            }
        };

        final long duration = mCallback.getAnimationDuration(mRecyclerView, ANIMATION_TYPE_SWIPE_SUCCESS,
                targetTranslateX - currentTranslateX, targetTranslateY - currentTranslateY);
        rv.setDuration(duration);
        mRecoverAnimations.add(rv);
        rv.start();
